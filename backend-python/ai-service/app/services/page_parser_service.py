...
class PageParserService:
    ...
    def _extract_metadata(
        self, soup: BeautifulSoup, page_url: Optional[str] = None
    ) -> Dict[str, Any]:
        """提取页面元数据"""
        metadata = {
            "title": "",
    ...
    def _extract_elements(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for cb in checkboxes:
            element = self._extract_element_info(
                cb, "CHECKBOX" if cb.get("type") == "checkbox" else "RADIO"
            )
            if element:
                elements.append(element)
    ...
    def _extract_element_info(
        ...
        try:
            ...
            if not element_text and element.name == "input":
                element_text = element.get("value", "") or element.get(
                    "placeholder", ""
                )
    ...
    def _extract_structure(self, soup: BeautifulSoup) -> Dict[str, Any]:
        """提取页面结构"""
        structure = {"sections": [], "forms": [], "navigation": []}
