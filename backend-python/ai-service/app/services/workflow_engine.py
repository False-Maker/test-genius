...
class WorkflowEngine:
    ...
    def _execute_nodes(
        ...
        for node_id in node_ids:
            ...
            if next_node_ids:
                # 递归执行下一个节点
                next_results = self._execute_nodes(next_node_ids, node_map, edge_map, context)
    ...
    def _handle_loop_branch(
        ...
        loop_items = []
        if isinstance(node_result, dict):
            if node_result.get("loop_type") == "for":
                loop_items = node_result.get("items", [])
            elif node_result.get("loop_type") == "while":
                # while循环需要根据条件决定是否继续
                # 这里简化处理，返回循环体节点
                return [loop_body_node]
        
        # 对于for循环，需要为每个迭代项执行循环体
        # 这里简化处理，只执行一次循环体，实际应该支持多次迭代
        # TODO: 实现完整的循环迭代逻辑
        if loop_items:
            # 将循环项存储到上下文中，供循环体使用
            context["current_loop_items"] = loop_items
            context["current_loop_index"] = 0
